"""FORGE Cell Calculator page.

Launch the multipage app with: streamlit run forge/gui/app.py
"""
    ...
    try:
        ...
        for ref_id in ref_ids:
            try:
                ...
                type_icon = {"Pouch": "[Pouch]", "Prismatic": "[Prismatic]", "Cylindrical": "[Cylindrical]"}.get(cell_type, "[Unknown]")
                data.append({
...
with st.sidebar:
    ...
    **Database:** {total_cells} cells
    Pouch: {len(pouch_cells)} | Prismatic: {len(prismatic_cells)} | Cylindrical: {len(cylindrical_cells)}
    ...
    for ref_id in available_refs:
        try:
            ...
            if ref_id in TIER1_CELLS:
                display_name = f"[Tier 1] {type_icon} {info.get('name', ref_id)}"
            elif info.get("confidence") == "high":
                display_name = f"[High] {type_icon} {info.get('name', ref_id)}"
            else:
    ...
    selected_display = st.selectbox(
        ...
        help="Load pre-defined cell parameters from the database. [Tier 1] = academic-validated, [High] = high confidence."
    )
        ...
        try:
            ...
            if selected_reference in TIER1_CELLS:
                ...
                **Tier 1 Academic-Validated**
                ...
                Publication: {tier1_info['authors']}, {tier1_info['journal']}

                DOI: https://doi.org/{tier1_info['doi']}

                {tier1_info['highlight']}
                """
                )
            ...
            with st.expander("Reference Info", expanded=False):
                ...
                conf_label = {"high": "HIGH", "medium": "MEDIUM", "low": "LOW"}.get(confidence, "UNKNOWN")
    ...
    if "loaded_reference" in st.session_state:
        ...
        if loaded_ref in TIER1_CELLS:
            st.success(f"Tier 1 loaded: **{loaded_ref}**")
        else:
